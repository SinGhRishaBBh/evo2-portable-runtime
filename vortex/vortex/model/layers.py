...
from vortex.model.hardware import supports_fp8, get_runtime_info

# ---------------------------------------------------------------------------
# Safe Transformer Engine imports
# ---------------------------------------------------------------------------
# TE is required for FP8 execution on Hopper/Ada Lovelace GPUs, but the
# runtime must be able to load without it for portable bf16 inference on
# older GPUs.  When TE is unavailable, TELinear falls back to nn.Linear.
# ---------------------------------------------------------------------------
try:
    ...
    _TELinearBase = nn.Linear  # Fallback: standard PyTorch linear layer
    te = None
    _HAS_TE = False
...
def set_format_recipe():
    """Create FP8 recipe for Transformer Engine.  Returns (None, None) if TE is unavailable."""
    if not _HAS_TE:
        return None, None
...
class TELinear(_TELinearBase):
    """
    Wrapper for Transformer-Engine's `Linear` layer with adaptive FP8 dispatch.

    Runtime behavior:
        - On Hopper/Ada Lovelace GPUs (CC >= 8.9): uses FP8 via te.fp8_autocast
        - On older GPUs or when TE is unavailable: falls back to bf16 execution
        - Base class is dynamically selected: TE Linear when available, else nn.Linear
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # FP8 is only usable when Transformer Engine is installed
        self.use_fp8_input_projections = use_fp8 and _HAS_TE
        if self.use_fp8_input_projections:
            self.fp8_format, self.fp8_recipe = set_format_recipe()

        # Track whether we've logged the FP8 dispatch decision (once per layer)
        self._logged_fp8_decision = False

        if _HAS_TE:
            # Full Transformer Engine initialization path
            super().__init__(
            ...
            # Fallback: standard nn.Linear (no TE features)
            super().__init__(
                in_features=input_size,
                out_features=output_size,
                bias=bias,
            )
            # Apply init_method to match TE behavior
            init_method(self.weight)
            # Cast to target precision
            self.to(dtype=params_dtype)

    def forward(self, x):
        # Adaptive FP8 dispatch: use FP8 only when both TE is available
        # and the device has FP8-capable Tensor Cores (CC >= 8.9).
        use_fp8 = self.use_fp8_input_projections and supports_fp8(x.device)

        # Log the dispatch decision once per layer instance for diagnostics
        if not self._logged_fp8_decision:
            if self.use_fp8_input_projections:
                log.info(
                    f"TELinear FP8 {'enabled' if use_fp8 else 'disabled (fallback to bf16)'} "
                    f"on {x.device}"
                )
            self._logged_fp8_decision = True
