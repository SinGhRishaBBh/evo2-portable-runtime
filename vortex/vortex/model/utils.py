...
def fixup_fp8_extra_states(module):
    ...
    Safe no-op for modules without fp8 state (e.g. when TE is not installed
    or the module is a standard nn.Linear fallback).
    """
    for child in module.children():
        fixup_fp8_extra_states(child)

    # Skip modules that don't have FP8 metadata (non-TE layers)
...
def fixup_te_workspace():
    ...
    Safe no-op if Transformer Engine is not installed.
    """
    try:
        import transformer_engine.pytorch.module.base as tebase
        import transformer_engine.pytorch.module.linear as telinear
    except ImportError:
        log.debug("Transformer Engine not installed — skipping workspace fixup.")
        return
