...
# ---------------------------------------------------------------------------
# Adaptive rotary embedding dispatch
# ---------------------------------------------------------------------------
# The Triton-based rotary kernel is significantly faster but requires a
# compatible GPU and Triton installation.  On systems where the Triton kernel
# cannot be loaded (e.g. V100, systems without Triton), we fall back to a
# pure-PyTorch implementation (apply_rotary_emb_torch, defined below).
# ---------------------------------------------------------------------------
try:
...
# ---------------------------------------------------------------------------
# Adaptive rotary dispatch function
# ---------------------------------------------------------------------------
_logged_rotary_fallback = False
...
def apply_rotary(
    ...
    Uses the Triton-accelerated kernel when available (fast path).
    Falls back to pure-PyTorch implementation on systems without Triton.

    Note: The PyTorch fallback does not support inplace operation, seqlen_offsets,
    cu_seqlens, or conjugate. These features are only available with Triton.
    """
